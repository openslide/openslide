                                                ...
                                                int32_t seek_location, int32_t len, GError **err)
{
  ...
  char *slide_image_xml_base64_modified = _openslide_remove_newline(slide_image_xml_base64);
  len = _openslide_get_trim_length(slide_image_xml_base64_modified, len);
  g_autofree unsigned char *slide_image_xml = g_malloc(BASE64_DECODE_OUT_SIZE(len));
  int slide_image_xml_length = _openslide_base64_decode(slide_image_xml_base64_modified, len, slide_image_xml);
  if (!slide_image_xml)
  {
    return false;
  }
                                             ...
                                             int len, GError **err)
{
  ...
  char *property_xml_base64_modified = _openslide_remove_newline(property_xml_base64);
  len = _openslide_get_trim_length(property_xml_base64_modified, len);
  g_autofree unsigned char *property_xml = g_malloc(BASE64_DECODE_OUT_SIZE(len));
  int property_xml_length = _openslide_base64_decode(property_xml_base64_modified, len, property_xml);
  if (!property_xml)
  {
    return false;
  }
