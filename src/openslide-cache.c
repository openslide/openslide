...
// eviction
// mutex must be held
static void possibly_evict(openslide_cache_t *cache, int64_t incoming_size) {
  g_assert(incoming_size >= 0);
  int64_t size = cache->total_size + incoming_size;
  int64_t target = cache->capacity;
...
openslide_cache_t *_openslide_cache_create(int64_t capacity_in_bytes) {
  ...
  *_entry = entry;
  ...
  if (size_in_bytes > cache->capacity) {
    ...
    _openslide_performance_warn_once(&cache->warned_overlarge_entry,
                                     ...
                                     "size %"PRId64" bytes", size_in_bytes);
    g_mutex_unlock(&cb->mutex);
    return;
  }
