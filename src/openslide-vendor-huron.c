...
struct huron_level {
	...
	struct huron_level *prev;
	GHashTable *missing_tiles;
	uint16_t compression;
};

static void huron_destroy_data(struct huron_ops_data *data,
struct huron_level **levels, int32_t level_count) {
	if (data) {
	...
	if (levels) {
		for (int32_t i = 0; i < level_count; i++) {
			if (levels[i]) {
				...
				g_slice_free(struct huron_level, levels[i]);
			}
		}
		g_free(levels);
	}
...
static void huron_destroy(openslide_t *osr) {
	struct huron_ops_data *data = osr->data;
	struct huron_level **levels = (struct huron_level **) osr->levels;
	huron_destroy_data(data, levels, osr->level_count);
}

static bool huron_render_missing_tile(struct huron_level *l,
	TIFF *tiff,
	uint32_t *dest,
...
static bool decode_tile_huron(struct huron_level *l,
	TIFF *tiff,
	uint32_t *dest,
	int64_t tile_col, int64_t tile_row,
	GError **err) {
	...
	if (g_hash_table_lookup_extended(l->missing_tiles, &tile_no, NULL, NULL)) {
		...
		return huron_render_missing_tile(l, tiff, dest,
...
static bool huron_read_tile(openslide_t *osr,
	cairo_t *cr,
struct _openslide_level *level,
	int64_t tile_col, int64_t tile_row,
	void *arg,
	GError **err) {
	struct huron_level *l = (struct huron_level *) level;
	struct _openslide_tiff_level *tiffl = &l->tiffl;
	TIFF *tiff = arg;
	...
	if (!tiledata) {
		...
		if (!decode_tile_huron(l, tiff, tiledata, tile_col, tile_row, err)) {
...
static bool huron_paint_region(openslide_t *osr, cairo_t *cr,
	int64_t x, int64_t y,
struct _openslide_level *level,
	int32_t w, int32_t h,
	GError **err) {
...
static const struct _openslide_ops huron_ops = {
	.paint_region = huron_paint_region,
	.destroy = huron_destroy,
};
...
static void huron_add_properties(openslide_t *osr, char **props) {
	if (*props == NULL) {
		return;
	}
...
static void propagate_missing_tile(void *key, void *value G_GNUC_UNUSED,
	...
	struct huron_level *next_l = data;
	struct huron_level *l = next_l->prev;
	struct _openslide_tiff_level *tiffl = &l->tiffl;
...
static bool huron_tile_decoding(struct huron_level *l,
	TIFF *tiff,
	GError **err) {
	// only for JP2K slides.
	// shouldn't affect RGB, but check anyway out of caution
	if (l->compression != HURON_COMPRESSION_JP2K_YCBCR &&
		l->compression != HURON_COMPRESSION_JP2K_RGB) {
		return true;
	}
	...
	bool ok = decode_tile_huron(l, tiff, dest, 0, 0, err);
...
struct _openslide_hash *quickhash1, GError **err) {
	struct huron_ops_data *data = NULL;
	struct huron_level **levels = NULL;
	...
	* page 14:
	*
	* The first image in the Huron BigTiff file is always the baseline image (full
	* resolution). This image is always tiled, usually with a tile size
	* of 256x256 pixels. The second image is always a thumbnail,
	...
	*/
	...
	data = g_slice_new0(struct huron_ops_data);

	levels = g_new0(struct huron_level *, level_count);
	int32_t i = 0;
	...
	do {
		...
		if (TIFFIsTiled(tiff)) {
			...
			struct huron_level *l = g_slice_new0(struct huron_level);
			struct _openslide_tiff_level *tiffl = &l->tiffl;
			if (i) {
				l->prev = levels[i - 1];
			}
	...
	// check for OpenJPEG CVE-2013-6045 breakage
	if (!huron_tile_decoding(levels[0], tiff, err)) {
		goto FAIL;
	}
	...
	char **props = g_strsplit(image_desc, "\n", -1);
	huron_add_properties(osr, props);
	g_strfreev(props);
...
FAIL:
	huron_destroy_data(data, levels, level_count);
	_openslide_tiffcache_put(tc, tiff);
