...
static bool vmic_read_tile(openslide_t *osr,
  ...
  if (!tiledata) { // in case of cache miss, retrieve tile from ZIP archive
    ...
    */
    ...
    if (zipindex!=-1) // note: getting an index "-1" means the tile doesn't exist. We used this as our empty marker
    {
      ...
      // the returned buffer @tiledata has been allocated with g_slice_alloc
      ...
      if (w!=tw || h !=th)
      {
          ...
          g_set_error(err, OPENSLIDE_ERROR, OPENSLIDE_ERROR_FAILED,
                  ...
                  (int)l->dz_level_id, (int)tile_row, (int)tile_col, (int)tw, (int)th, (int)w, (int)h);
  ...
  if (tiledata)  {
    ...
    // no tile at this position => don't need to do anything because cairo surface is pre-filled with transparent pixels
  }
...
static xmlNode* _openslide_xml_find_node(xmlNode *node, const xmlChar* name)  {
  ...
  while (cur!=NULL) {
      if(xmlStrcmp(cur->name, name)==0) { break; }
      cur = cur->next;
  }
  return cur;
}
...
static struct vmicinfo* vmic_open_container(const char *vmic_filename, bool detection_only, GError **err) 
{
  ...
  vmic->zip_entries = zip_get_num_entries(vmic->zi.handle, 0);
...
FAIL_AND_CLOSE:
  if (zo) _openslide_zip_close_archive(&vmic->zip_outer);
...
static xmlDoc *_openslide_zip_parse_xml_file(struct _openslide_ziphandle *zh,
                                      const char *filename,
                                      zip_flags_t flags,
                                      GError **err,
  ...
  if (xml_file_id==-1) {
    ...
    g_set_error(err, OPENSLIDE_ERROR, OPENSLIDE_ERROR_FAILED,
                "Cannot locate XML description file \"%s\" in container. libzip message:\"%s\"", filename, zip_error_strerror(ze) );
    goto FINISH;
  }
...
static bool vmic_get_properties(openslide_t *osr, struct _openslide_hash *quickhash, GError **err) {
  ...
  dzz_find_key_file(vmic->zi.handle, dzz );
  //g_debug("deepzoom key file=%s, folder=%s\n", dzz->key_filename, dzz->folder_name);
  
  // Parsing DEEPZOOM properties
  xmldoc = _openslide_zip_parse_xml_file(&vmic->zi, dzz->key_filename, ZIP_FL_ENC_RAW|ZIP_FL_NOCASE, err, quickhash);
  if (!xmldoc) goto FAIL;
  ...
  xmldoc = _openslide_zip_parse_xml_file(&vmic->zi, _PRECIPOINT_PROPS_FILENAME, ZIP_FL_ENC_RAW|ZIP_FL_NOCASE, err, quickhash);
  if (!xmldoc) goto FAIL;
...
static bool vmic_create_levels(openslide_t *osr, GError **err)
{
  ...
  for (dz_level_id = dzi->dz_level_count-1; dz_level_id>=dzi->dz_one_tile_level; dz_level_id--) {
    ...
    l->base.h = h;
    l->base.downsample = downsample; // 2^level
...
static bool precipoint_open( openslide_t *osr, const char *filename,
  ...
  gpointer hashbuf; gsize bufsize;
