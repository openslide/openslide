...

{
    ...
    // extract MapFile
    tmp = g_key_file_get_string( vmu_file,
				GROUP_VMU,
				KEY_MAP_FILE,
				NULL );
    if ( tmp ) {
	...
	if ( vf->end_in_file == -1 ) {
	    g_warning( "Can't read file size for VMU image file %s", 
		      vf->filename );
	    fclose( f );
	    goto FAIL;
	}
	...
	if ( !fread( &key, sizeof( key ), 1, f ) ) {
	    ...
	    g_warning( "Unexpected end of file reading header of %s.",
		       vf->filename );
	    fclose( f );
	    goto FAIL;
	...
	vf->chunk_table = 
	    ( int64_t ** ) g_malloc( sizeof( int64_t ) * vf->h );
	vf->chunk_table[0] = 
